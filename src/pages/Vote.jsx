...
export default function Vote() {
    ...
    const fetchPolls = async () => {
      ...
      try {
        ...
        if (showFilter) {
          ...
          fetchedPolls = await getPollsByGender(showFilter, 20, reportedPollIds)
        } else {
  ...
  const { swipeOffset, isSwiping, handlers: swipeHandlers } = useSwipe({
    onSwipeLeft: () => {
  ...
  const swipeDirection = swipeOffset > 30 ? 'A' : swipeOffset < -30 ? 'B' : null
        ...
        <div className="text-center">
          ...
          <div className="flex gap-2 justify-center mt-3">
            <button
              ...
              className="py-2 px-4 rounded-full border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 text-gray-700 dark:text-gray-300 text-sm"
            >
              Women's outfits
            </button>
            <button
    ...
    return (
      <div className="text-center space-y-4 py-12">
        ...
        <button
          ...
          className="py-2 px-4 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg text-gray-700 dark:text-gray-300"
        >
          Try Again
        </button>
      </div>
  ...
  if (!currentPoll || polls.length === 0) {
    return (
      <EmptyState
        icon="🎉"
        title={voteCount > 0 ? "You've voted on all polls!" : "No polls available"}
        description={
          voteCount > 0
            ? `You've helped ${voteCount} ${voteCount === 1 ? 'person' : 'people'} today. Check back later for more polls.`
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <button
        onClick={handleSkip}
        disabled={isVoting}
        className="w-full py-2 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 text-sm transition-colors disabled:opacity-50"
      >
        Skip this poll
      </button>
